...
class OcrInferencer:
    ...
    def _infer(self, single_outputdir_data):
        ...
        for img_path in single_outputdir_data['img_list']:
            ...
            if self.cfg['add_info']:
                ...
                self._save_recog_image(outputdirpath, os.path.basename(img_path),
                                      single_image_file_output[0]["img"],
                                      single_image_file_output[0]["json"])
            else:
                self._save_pred_json(outputdirpath,os.path.basename(img_path),
    ...
    def _save_recog_image(self, output_dir, orig_img_name, img, pred_json):
        """
        認識結果の矩形を描画した画像を保存します。
        ...
        try:
            ...
            print("[ERROR] Recognition image save error: {0}".format(err), file=sys.stderr)
            raise OSError
        return
