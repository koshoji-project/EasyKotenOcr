...
:: 4. インストール先の選択 (CLIプロンプト)
set "DEFAULT_INSTALL_DIR=c:\easykotenocr"

:SET_INSTALL_DIR
echo.
echo 既定のインストール先は %DEFAULT_INSTALL_DIR% です。
set /p "CHOICE=規定のインストール先でよいですか？ (Y/n): "
set "NEGATIVE=0"
if /i "!CHOICE!"=="n" set "NEGATIVE=1"
if "!CHOICE!"=="ｎ" set "NEGATIVE=1"
if "!CHOICE!"=="Ｎ" set "NEGATIVE=1"
if !NEGATIVE! equ 1 (
    goto INPUT_CUSTOM_PATH
)
...
:INPUT_CUSTOM_PATH
echo.
echo 既定のインストール先: %DEFAULT_INSTALL_DIR%
echo これを編集して希望するインストール先のフォルダー名を入力してください。
echo (ドライブ名が省略された場合は C:\ が起点となります)
set /p "RAW_PATH=インストール先フォルダー名: "

:: ドライブレターの有無を確認 (: が2文字目にあるか)
set "CHECK_DRIVE=!RAW_PATH:~1,1!"
if "!CHECK_DRIVE!"==":" (
    set "INSTALL_DIR=!RAW_PATH!"
) else (
    :: ドライブ指定がない場合は C:\ を起点にする
    :: 先頭が \ の場合は重複しないように調整
    set "FIRST_CHAR=!RAW_PATH:~0,1!"
    if "!FIRST_CHAR!"=="\" (
        set "INSTALL_DIR=C:!RAW_PATH!"
    ) else (
        set "INSTALL_DIR=C:\!RAW_PATH!"
    )
)
...
:CONFIRM_PATH
echo.
echo 設定されたインストール先: !INSTALL_DIR!
set /p "CHOICE=これでよいですか？ (Y/n): "
set "NEGATIVE=0"
if /i "!CHOICE!"=="n" set "NEGATIVE=1"
if "!CHOICE!"=="ｎ" set "NEGATIVE=1"
if "!CHOICE!"=="Ｎ" set "NEGATIVE=1"
...
if exist "!INSTALL_DIR!" (
    ...
    if !FOLDER_EMPTY! equ 0 (
        echo.
        echo [警告] 選択されたフォルダー "!INSTALL_DIR!" は空ではありません。
        set /p "DEL_CHOICE=フォルダー内のファイルをすべて削除してよいですか？ (Y/n): "
        set "NEGATIVE=0"
        if /i "!DEL_CHOICE!"=="n" set "NEGATIVE=1"
        if "!DEL_CHOICE!"=="ｎ" set "NEGATIVE=1"
        if "!DEL_CHOICE!"=="Ｎ" set "NEGATIVE=1"
        if !NEGATIVE! equ 1 (
            goto INPUT_CUSTOM_PATH
        )
        echo フォルダー内のすべてのファイルとサブフォルダーを削除しています...
        rd /s /q "!INSTALL_DIR!"
        mkdir "!INSTALL_DIR!"
        echo インストール先フォルダーの中のファイルとサブフォルダーの削除が完了しました。
    )
)
echo.
